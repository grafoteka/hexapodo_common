...
bool moveLegs(std::vector<int> tripod_one, float alpha_rads, bool moveLegsConsigna, float leg_velocity)
{
  ROS_INFO("MoveLegs");
  if(moveLegsConsigna == false)
  {
    ...
    {
      if(moveLegsConsignaPatasAlcanzada.at(i) == false)
      {
        pata[tripod_one.at(i)]->setVelocity(leg_velocity);
      }
    }
   }
  else
...
//      ROS_INFO("%.2f", alpha_rads);
      ...
      distancia = fabs(alpha_rads - pos_pata_actual );
      ROS_INFO("Pata %d -- Con: %.2f  Lectura -> %.2f - distancia %.2f", i, alpha_rads, pos_pata_actual, distancia);
      if(distancia <= 0.05)
      {
        pata[tripod_one.at(i)]->setVelocity(0.0);
//        pata[tripod.at(i)]->setPosition(alpha_rads);
        moveLegsConsignaPatasAlcanzada.at(i) = true;
        ROS_INFO("Pata %d consigna alcanza, velocidad = %.2f", i, leg_velocity);
      }
     }
    }
    if(std::all_of(moveLegsConsignaPatasAlcanzada.begin(), moveLegsConsignaPatasAlcanzada.end(), [](bool posicion_origen) {return posicion_origen;}))
    {
      ROS_INFO("TRANSICION");
      //moveLegsConsigna = true;
      for(int i = 0; i < moveLegsConsignaPatasAlcanzada.size(); i++)
      {
        moveLegsConsignaPatasAlcanzada.at(i) = false;
      }
      return true;
    }
...
//      tripodOneStateNext = flight;
      angulo = alpha_rads;
      flag = moveLegs(tripod_one, angulo, false, leg_ground_velocity);
...
//      }
    ...
    {
      ROS_INFO("Origin To TRANSITION");
      angulo = 2 * PI - alpha_rads;
      flag = moveLegs(tripod_one, angulo, false, leg_flight_velocity);
      tripodOneStateActual = transition;
      tripodOneStateLast = origin;
...
//      moveLegs(tripod_one, angulo, false);
  ...
  case transition:
    // Origin -> LAND
    flag = moveLegs(tripod_one, angulo, true, leg_ground_velocity);
    if(flag)
    {
...
//      ROS_INFO("Land To TRANSITION");
      ROS_INFO("LAND");
      angulo = alpha_rads;
      moveLegs(tripod_one, angulo, false, leg_ground_velocity);
...
//      ROS_INFO("Flight to TRANSITION");
      ROS_INFO("FLIGHT");
      angulo = 2 * PI - alpha_rads;
      moveLegs(tripod_one, angulo, false, leg_flight_velocity);
      tripodOneStateActual = transition;
      tripodOneStateLast = flight;
      tripodOneStateNext = land;
...
//  {
//    if(tripodOneConsignaAlcanzada == false){
//      for(int i = 0; i < 3; i++)
//      {
//        if(patas_posicion_actual.at(tripod_one.at(i)) < alpha_rads)
//        {
//          pata[tripod_one.at(i)] -> setVelocity(leg_ground_velocity);
//        }
//        if(patas_posicion_actual.at(tripod_one.at(i)) >= alpha_rads)
//        {
//          pata[tripod_one.at(i)] -> setVelocity(0.0);
//          tripodOnePatasPosicionAlcanzada.at(i) = true;
//        }
//      }
//      if(std::all_of(tripodOnePatasPosicionAlcanzada.begin(), tripodOnePatasPosicionAlcanzada.end(), [](bool posicion_origen) {return posicion_origen;})){
//        ROS_INFO("Posicion despegue = TRUE");
//        tripodOneConsignaAlcanzada = true;
//      }
//    }
//  }
