...
//    ROS_INFO("Valor varibale = %d", legs_read_flag);
    if(legs_read_flag)
    {
      ...
      for(int i = 0; i < 3; i++)
      {
        float error = (hexapodo_stand_by.pata[tripod_two.at(i)]->getPos() - (2 * PI - total_angle_rads / 2));
        ...
        if (abs(velocity) < 0.1)
        {
          hexapodo_stand_by.pata[tripod_two.at(i)]->setPosition(2 * PI - (total_angle_rads / 2));
